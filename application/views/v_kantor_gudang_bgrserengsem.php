...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kantor_gudang_bgrserengsem/ajax_list')?>",
            "type": "POST"
        },
...
function add_kantor_gudang_bgrserengsem()
{
    save_method = 'add';
    ...
    $('.modal-title').text('TAMBAH kantor_gudang_bgrserengsem'); // Set Title to Bootstrap modal title
}

function edit_kantor_gudang_bgrserengsem(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('kantor_gudang_bgrserengsem/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('kantor_gudang_bgrserengsem/ajax_add')?>";
    } else {
        url = "<?php echo site_url('kantor_gudang_bgrserengsem/ajax_update')?>";
    }
...
function delete_kantor_gudang_bgrserengsem(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('kantor_gudang_bgrserengsem/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
