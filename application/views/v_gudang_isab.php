...
<div class="container" style="border: 1px solid #cccccc; background-color: #F5F5F5; " >
  <section class="content-header"><br>
    <h2 class="text-center" style="font-size: 3vw;"><b>GUDANG ISAB</b></h2><br><br>
  </section>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('gudang_isab/ajax_list')?>",
            "type": "POST"
        },
...
function add_gudang_isab()
{
    ...
    $('.modal-title').text('TAMBAH gudang_isab'); // Set Title to Bootstrap modal title
}

function edit_gudang_isab(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('gudang_isab/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('gudang_isab/ajax_add')?>";
    } else {
        url = "<?php echo site_url('gudang_isab/ajax_update')?>";
    }
...
function delete_gudang_isab(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('gudang_isab/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<form method="post" accept-charset="utf-8" action="<?php echo base_url()?>c_gudang_isab/cetak">
...
<form method="post" accept-charset="utf-8" action="<?php echo base_url()?>c_gudang_isab/cetak_full">
    <div class="form-group">
        <div  style="width: 170px;  float: left;">
